...
object ZIOModel {
  ...
  final case class ZIO[-R, +E, +A](run: R => Either[E, A]) {
    ...
    def zip[R1 <: R, E1 >: E, B](that: ZIO[R1, E1, B]): ZIO[R1, E1, (A, B)] =
      ZIO(r => self.run(r).flatMap(a => that.run(r).map(b => (a, b))))

    def either: ZIO[R, Nothing, Either[E, A]] =
      ZIO(r => self.run(r).fold(e => Right(Left(e)), a => Right(Right(a))))
...
object PrintSequenceZip extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("Hello").zip(putStrLn("World").zip(putStrLn("!"))) as ExitCode.success
}
...
object PrintSequence extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    (putStrLn("Hello") zipRight putStrLn("World") *> putStrLn("!")).exitCode
}
...
object SimpleDuplication extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] = {
    val effect = putStrLn("Hello again").repeatN(2)
    (putStrLn("Hello") *> effect).exitCode
  }
...
object FlatMap extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    (putStrLn("What is your name?") *> readLine.flatMap(r => putStrLn(s"Your name is: $r"))).exitCode
}
...
object PromptName extends App {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("What is your name?")
      .flatMap(_ => readLine.flatMap(name => putStrLn(s"Your name is: ${name}")))
