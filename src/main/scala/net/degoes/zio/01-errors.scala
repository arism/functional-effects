...
object ErrorRecoveryOrElse extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.orElse(putStrLn("Oh Yeah !")) as ExitCode.success
}
...
object ErrorShortCircuit extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.orElse(putStrLn("fallback")) as ExitCode.success
}
...
object ErrorRecoveryFold extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.fold(putStrLnErr(_),putStrLn(_)) as ExitCode.success
}
...
object ErrorRecoveryCatchAll extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.catchAll(putStrLnErr(_)) as ExitCode.success
}
...
object ErrorRecoveryFoldM extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.foldM(putStrLnErr(_),putStrLn(_)) as ExitCode.success
}
...
object AlarmApp extends App {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    (for {
      ...
      _ <- putStrLn(s"Sleeping for ${duration.getSeconds} seconds")
      _ <- ZIO.sleep(duration)
      _ <- putStrLn("Time to wakeup!!!")
    } yield ExitCode.success).refineToOrDie[Nothing]
}
...
object Sandbox extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    composed.sandbox.mapError(c => putStrLnErr(c.prettyPrint)).ignore as ExitCode.success
}
